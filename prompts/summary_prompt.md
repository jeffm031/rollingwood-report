...
- **Roster is authoritative for spelling.** For every person you name in the summary, resolve their transcript spelling to a canonical entry in the roster. When you find a match, use the canonical spelling — the canonical form, not an alias form, is the only valid rendering in the summary body. An alias listed in a roster entry's `aliases` field identifies a variant to recognize in the transcript, not a variant to use in output. Do not announce or footnote silent corrections — just write the canonical form.
